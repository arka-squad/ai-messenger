...
from ...application import BoiteIndisponible, Messagerie
...
def etat(messagerie: Messagerie, compte: str) -> Dict[str, Any]:
...
def creer_serveur(messagerie: Messagerie, compte: str, port: int = 0,
                  front: Optional[str] = None) -> http.server.HTTPServer:
    """Un serveur prêt à `serve_forever()`. Port 0 : un port libre est choisi."""
    serveur = _Serveur(("127.0.0.1", port), _gestionnaire(messagerie, compte, front))
    return serveur


def servir(messagerie: Messagerie, compte: str, port: int, front: Optional[str],
           ouvrir_navigateur: bool = True, lie_au_parent: bool = False) -> int:
    ...
    try:
        serveur = creer_serveur(messagerie, compte, port, front)
    except OSError:
...
def _gestionnaire(messagerie: Messagerie, compte: str, front: Optional[str]):
    class Gestionnaire(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            try:
                if chemin == "/api/boite":
                    return self._json(200, etat(messagerie, compte))
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if urllib.parse.urlsplit(self.path).path != "/api/statut":
                return self._erreur(404, "introuvable")
